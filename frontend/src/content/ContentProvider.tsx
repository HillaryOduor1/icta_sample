  ...
  if (responseData && responseData.data) {
    ...
    if (Array.isArray(dataContent)) {
      if (dataContent.length === 0) {
        // Log without exposing structure
        console.warn('Content data array is empty');
...
const checkContentVersion = async (): Promise<number | null> => {
  try {
    ...
    if (!response.ok) {
      // Log without exposing status details
    ...
    console.warn('Version check unavailable');
    return null;
  }
};
...
export function ContentProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadContent = React.useCallback(async (force = false) => {
      ...
      if (cached) {
        try {
          ...
          if (Date.now() - timestamp < CACHE_TTL) {
            // Log without exposing tenant or version details
          ...
          // Silently handle cache parse errors
        }
      }
    }
    ...
    try {
      ...
      // Log generic message without exposing tenant or URL
      ...
      // Log success without exposing data structure
      ...
      if (loadedContent && Object.keys(loadedContent).length > 0) {
        ...
        if (newVersion > currentVersion || force) {
          ...
          // Log without exposing version details
          console.log('Content is up to date');
        }
      } else {
        console.warn('No valid content received, using fallback');
  ...
  const checkVersionAndReload = React.useCallback(async () => {
    ...
    if (serverVersion && serverVersion > currentVersion) {
      console.log('Content update available, reloading...');
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Check silently without logging
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === CONTENT_VERSION_KEY && e.newValue) {
        ...
        if (newVersion > currentVersion) {
          console.log('Cross-tab update detected, reloading...');
          loadContent(true);
        }
      }
    };
  ...
  const resetContent = async () => {
    console.log('Resetting content to default...');
  ...
  const loadContent = React.useCallback(async (force = false) => {
    ...
    try {
      ...
      if (loadedContent && Object.keys(loadedContent).length > 0) {
        ...
        console.warn('No valid content received from backend, using static fallback');
        setContent(staticDefault as SiteContent);
      }
    } catch (error) {
